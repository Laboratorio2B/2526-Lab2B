#include "xerrori.h"

// primo esempio di utilizzo di thread
// conteggio dei primi con thread multipli

// mostra l'uso degli attributi di un thread
// creando thread in modalità detached 
// l'effetto è quello di rendere non valida 
// l'operazione pthread_join: in questo modo
// testtiamo l'uso di xpthread_join in xerrori.c


// mostra come usare una struct per condividere dei parametri 
// di input e output tra thread principale e ausiliari

// si tratta della situazione piu semplice in cui il lavoro 
// può essere suddiviso in più parti indipendenti;


//Prototipo test di primalità
bool primo(int n);

// struct usata per passare argomenti ai thread
typedef struct {
  int start;            // intervallo dove cercare i primo 
  int end;              // parametri di input
  int somma_parziale;   // parametro di output
  pthread_barrier_t *b; // barriera per la sincronizzazione
} dati;

// funzione passata a pthred_create
void *tbody(void *v) {
  dati *d = (dati *) v;
  int primi = 0;
  fprintf(stderr, "Conto i primi fra %d e %d\n", d->start, d->end);
  // cerco i primi nell'intervallo assegnato
  for(int j=d->start;j<d->end;j++) {
      if(primo(j)) primi++; 
  }
  fprintf(stderr, "Il thread che partiva da %d ha terminato\n", d->start);
  d->somma_parziale = primi;
  xpthread_barrier_wait(d->b, __LINE__,__FILE__);
  pthread_exit(NULL);
}

int main(int argc,char *argv[])
{
  if(argc!=3) {
    fprintf(stderr,"Uso\n\t%s m num_threads\n", argv[0]);
    exit(1);
  }
  // conversione input
  int m= atoi(argv[1]);
  if(m<1) termina("limite primi non valido");
  int p= atoi(argv[2]);
  if(p<=0) termina("numero di thread non valido");

  // creazione thread ausiliari
  pthread_t t[p];   // array di p indentificatori di thread 
  dati d[p];        // array di p struct che passerò ai p thread
  pthread_attr_t attr;
  pthread_attr_init(&attr); // manca il controllo del return value!
  pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
  pthread_barrier_t barriera;
  xpthread_barrier_init(&barriera,NULL,p+1,__LINE__,__FILE__); 
  for(int i=0; i<p; i++) {
    int n = m/p;  // quanti numeri verifica ogni thread + o - 
    d[i].start = n*i; // inizio range thread i
    d[i].end = (i==p-1) ? m : n*(i+1);
    d[i].b = &barriera;
    xpthread_create(&t[i], &attr, &tbody, &d[i],__LINE__, __FILE__); 
  }
  // attendo che i thread abbiano finito
  xpthread_barrier_wait(&barriera,__LINE__,__FILE__); 
  int somma = 0;        // variabile dove accumulo il numero di primi
  for(int i=0;i<p;i++) {
    // xpthread_join(t[i],NULL,__LINE__, __FILE__);
    somma += d[i].somma_parziale;
  }
  // restituisce il risultato 
  printf("Numero primi tra 1 e %d (escluso): %d\n",m,somma);
  return 0;
}



// restituisce true/false a seconda che n sia primo o composto
bool primo(int n)
{
  if(n<2) return false;
  if(n%2==0) {
    if(n==2)  return true;
    else return false; 
  }
  for (int i=3; i*i<=n; i += 2) 
    if(n%i==0) return false;
  return true;
}

