...
def main(nomedir):
  ...
  diz = {}
  dizionario_lunghezze(nomedir,diz,set())
  print(diz)
...
def dizionario_lunghezze(nome,diz,dir_esplorate):
 ...
 di quella lunghezza tra quelli nella directory nome e sottodirectory """
  ...
  for f in listafile:
    ...
    if not os.path.isdir(nomecompleto):
      ...
      dizionario_lunghezze(nomecompleto,diz,dir_esplorate)
  # ciclo for su i file di questa directory terminato     
  print(f"End: {nome}",file=sys.stderr)


def cmpshell(a,b):
  """Confronta i due file usando cmp della shell restituisce il return value di cmp"""
