...
def aggiorna_classifica(c,s1,g1,s2,g2):
  """Aggiorna per s1 e s2 i punti e i gol fatti e subiti. 
...
def crea_classifica(f):
  """legge le righe dei risultati dal file f
  e aggiorna e crea la classifica risultante"""
  c = {}     # crea classifica vuota
  # leggo le linee di f una alla volta
  # ad ogni iterazione la linea è in "linea"
  for linea in f:
...
def stampa_squadra(nome,punti,golf,gols):
  ...
  # si usano le seguenti istruzioni di formattazzione: 
...
def main(nomefile):
  with open(nomefile,"r") as f:
    ...
    c = crea_classifica(f)
  # siamo all'uscita dal blocco with
  # il file viene automaticamente chiuso

  # ottengo la lista delle squadre partecipanti
  # trasformado in lista l'insieme delel chiavi di c
