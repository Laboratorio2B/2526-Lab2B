...
class Miofile:
  ...
  def __lt__(self,other):
    "confronta dimensioni e a parità di dimensione il nome e mtime"
    return (-self.size,self.path,self.mtime) < (-other.size,other.path,other.mtime)
...
def main(nomedir,g,regex=None):
  ...
  elenco = cerca_recenti(nomedir,limite,set(),regex)
  print("------------------------") 
    ...
    if not os.path.isdir(nomecompleto):
      ...
      if not miof.precedente_a(datalimite):
        if regex:
          ...
          # se nomecompleto ha un match con regex
