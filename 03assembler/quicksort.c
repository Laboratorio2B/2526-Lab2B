...
// Scopo del programma:
//   funzione quicksort e misura del tempo di esecuzione
//   se definita la costante USA_ARM viene invocato qs_arm() invece di quicksort

// include e define per misurare il wall clock time 
#include <unistd.h>
#include <sys/times.h>
...
int partition(int a[], int n)
{
  assert(n>1);
  #ifdef RANDOM_PIVOT
  // scelgo pivot in posizione random (da preferire!!!)
  ...
  #else
  int pivot=a[0];            // prendo a[0] come pivot (per imitare la versione arm)
  #endif 

  // procedura di partizionamento
  // l'elemento pivot in a[0] svolge anche la funzione di sentinella  
